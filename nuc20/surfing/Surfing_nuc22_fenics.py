...
def sigmavm(sig,v):
	...
	return sqrt(3/2*(inner(sig-1/3*tr(sig)*Identity(len(v)), sig-1/3*tr(sig)*Identity(len(v))) + (1/9)*tr(sig)**2 ))
...
Wv=pen/2*((abs(z)-z)**2 + (abs(1-z) - (1-z))**2 )*dx(0)
Wv2=conditional(le(z, 0.05), 1, 0)*10*pen/2*( 1/4*( abs(z_prev-z)-(z_prev-z) )**2 )*dx(0)
if phase_model==1:
	R_z = y*2*z*(psi11)*dx(0)+ y*(ce)*dx(0) +3*k/8*(y*(-1)/lch + 2*lch*inner(grad(z),grad(y)))*dx(0) + derivative(Wv2,z,y) #+ derivative(Wv,z,y)  #linear model
else:
...
terminate=0
printsteps=5  #Number of incremental steps after which solution will be stored
...
while t-stepsize < T:	
	...
	while stag_iter<800 and unorm_stag > 1e-9:
		...
		if comm_rank==0:
			with open('/fenics/Stag_stopping_crit_' + str(step) + '.txt', 'a') as rfile:
	...
	if terminate==1:
		...
		if comm_rank==0:
			print(Fx)
			with open('/fenics/SurfingG_eps12_check.txt', 'a') as rfile:
				rfile.write("%s %s\n" % (str(t), str(Jintegral)))
			
			
		####Plot solution on incremental steps
		if step % printsteps==0:
			file_results = XDMFFile( "/fenics/SurfingG_eps12d_check_" + str(step) + ".xdmf" )
