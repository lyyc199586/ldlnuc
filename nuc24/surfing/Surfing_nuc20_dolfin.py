...
def lefttop(x):
    ...
    return abs(x[0]) < 1e-4 and abs(x[1] + 5) < 1e-4
...
Wv2 = conditional(le(z, 0.05), 1, 0) * 10 * pen / 2 * (1 / 4 * (abs(z_prev - z) - (z_prev - z)) ** 2) * dx(0) # NOTE: penalty enforcing conditional bounds
if phase_model == 1:
    R_z = (
        y * 2 * z * (psi11) * dx(0)
        + y * (ce) * dx(0)
    ...
    R_z = (
        y * 2 * z * (psi11 + ce) * dx(0) + k * (y * (z - 1) / lch + lch * inner(grad(z), grad(y))) * dx(0) + derivative(Wv2, z, y)
    )  # quadratic model
...
while t - stepsize < T:
    ...
    if terminate == 1:
        ...
        if comm_rank == 0:
            print(Fx)
            with open("./out/Surfing_nuc20_t_Jint.txt", "a") as rfile:
                rfile.write("%s %s\n" % (str(t), str(Jintegral)))

        ####Plot solution on incremental steps
        if step % printsteps == 0:
            file_results = XDMFFile("./out/surfing_nuc20_" + str(step) + ".xdmf")
